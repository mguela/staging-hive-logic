...
test('default root deadline is exactly bounded at fifteen seconds and closes idempotently', () => {
  ...
  assert.equal(SERVER_DEADLINE.totalMs, 15_000);
  assert.equal(SERVER_DEADLINE.composerMs, 9_000);
  assert.equal(SERVER_DEADLINE.contextMs, 2_500);
  assert.equal(deadline.deadlineAt >= before + 15_000, true);
  assert.equal(deadline.deadlineAt <= after + 15_000, true);
  assert.equal(deadline.remainingMs() <= 15_000, true);
...
test('bounded store separates the exact 4 second RPC fence from the full attempt lease', async () => {
  ...
  assert.equal(deadline.deadlineAt, 25_000);
  assert.deepEqual(receivedDeadlines, [{ deadlineAt: 14_000, attemptDeadlineAt: 25_000 }]);
  ...
  assert.equal(timers[0].delay, 15_000, 'root timer retains the total request budget');
...
test('a typo in a dashboard field falls back to the default instead of taking the route down', () => {
  for (const bad of ['', 'true', 'soon', '-1', '0', '999999999', null, undefined, {}, []]) {
    ...
    assert.equal(resolved.totalMs, 15_000, `REINA_PILOT_TOTAL_MS=${JSON.stringify(bad)}`);
  }
...
test('a stage can never be given longer than the request it belongs to', () => {
  ...
  assert.equal(resolved.composerMs, 9_000, 'out of range for this total, so the default stands');
