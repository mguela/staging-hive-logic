...
import { isPublicApiPath, decideAccess, verifyUserBearer, checkCronSecret } from './api/_lib/guard.js';
...
export default async function middleware(request) {
  ...
  if (pathname === '/api/selftest-report' && request.method === 'GET') return undefined;
  ...
  const verify = hasValidCronSecret ? false : await verifyUserBearer(authHeader);
  ...
  return new Response(
    ...
    { status: decision.status || 401, headers: { 'content-type': 'application/json' } },
  );
}
