-- sql/049_marketing_new_lead_followup_type.sql
-- Phase 14 (Lifecycle marketing playbooks) -- new-lead follow-up playbook.
-- ADDITIVE ONLY: widens the existing campaigns.type CHECK constraint to
-- also allow 'new_lead_followup', so a real campaign row can eventually be
-- created for this playbook (api/marketing.js's resource=lifecycle_candidates
-- GET endpoint reads this same real column, via
-- `fetchAllRows('campaigns', '?select=id&type=eq.new_lead_followup')`, to
-- detect leads already contacted for this playbook -- until this migration
...
-- (added 'post_job_thank_you') and sql/048_marketing_service_anniversary_type.sql
-- (added 'service_anniversary'). All three are additive and independent;
-- apply in any order, or combine into one statement when the Phase 14
