// api/bookkeeping/purchase-orders/reject.js
...
export default async function handler(req, res) {
  ...
  try {
    const { rejectPurchaseOrder } = await _load_purchase_orders();
        const input = typeof req.body === 'string' ? JSON.parse(req.body || '{}') : (req.body || {});
    const rejected = await updatePurchaseOrder(actor.companyId, input.poId, po => rejectPurchaseOrder(po, actor, input.reason));
    res.status(200).json({ ok: true, purchaseOrder: rejected });
  } catch (error) {
    res.status(422).json({ ok: false, error: error.message || 'Could not reject this purchase order.' });
  }
}
