-- Live migration version 20260818000148. Keep crew payroll rows and the field tech's activity row transactional.
-- The first implementation closed old sessions with one REST request and
-- opened replacements with another. A failed insert could therefore leave an
-- entire crew clocked out. These service-role-only RPCs make each user action
-- one database transaction; any error rolls every write back.
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(p_rows) as item
    group by item->>'employee_jid'
    having count(*) > 1
  ) then
    raise exception 'A crew member may appear only once in a group clock action.';
  end if;

  select coalesce(min(row_data.clock_in), clock_timestamp())
    into v_clock_at
...
begin
  ...
  insert into public.job_time_entries (
    ...
    v_tech_id,
    nullif(p_entry->>'tech_name', ''),
    nullif(p_entry->>'job_ref', ''),
    nullif(p_entry->>'visit_ref', ''),
    nullif(p_entry->>'client_ref', ''),
    p_entry->>'kind',
    coalesce((p_entry->>'whole_team')::boolean, false),
    v_now,
    nullif(p_entry->>'note', '')
  ) returning * into v_entry;

  if jsonb_array_length(p_crew_rows) > 0 then
    select coalesce(jsonb_agg(to_jsonb(clock_row)), '[]'::jsonb)
      into v_clock
...
create or replace function public.hl_field_time_stop(
  p_tech_id uuid,
...
begin
  ...
  if jsonb_array_length(p_crew_jids) > 0 then
    update public.hl_clock as clock_row
       set clock_out = v_now
...
revoke all on function public.hl_field_time_stop(uuid, jsonb) from public, anon, authenticated;
...
grant execute on function public.hl_field_time_stop(uuid, jsonb) to service_role;

comment on function public.hl_clock_crew_in(jsonb) is
  'Service-only atomic replacement of open hl_clock sessions for a crew.';
comment on function public.hl_field_time_start(jsonb, jsonb) is
  'Service-only atomic field activity start plus optional whole-crew clock-in.';
comment on function public.hl_field_time_stop(uuid, jsonb) is
  'Service-only atomic field activity stop plus optional whole-crew clock-out.';
