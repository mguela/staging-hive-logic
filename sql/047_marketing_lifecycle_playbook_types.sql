-- sql/047_marketing_lifecycle_playbook_types.sql
-- Phase 14 (Lifecycle marketing playbooks) -- first playbook type.
-- ADDITIVE ONLY: widens the existing campaigns.type CHECK constraint to
-- also allow 'post_job_thank_you', so a real campaign row can eventually be
-- created for the post-job-thank-you playbook (api/marketing.js's new
-- resource=lifecycle_candidates GET endpoint reads this same real column,
-- via `fetchAllRows('campaigns', '?select=id&type=eq.post_job_thank_you')`,
-- to detect jobs already thanked -- until this migration is applied that
-- query always returns zero rows, which is an honest state, not a bug).
--
