// api/bookkeeping/change-orders/approve.js — sent -> approved. Body: { id }.
// Only the controller/admin role may approve on the company's behalf; a
// client-side approval link (future work) would need its own trust path —
// this route is for internal recording of a client's approval, not a
...
export default async function handler(req, res) {
  ...
  if (actor.role !== 'controller') { res.status(403).json({ ok: false, error: 'Only a controller/admin may record a change order approval.' }); return; }
  ...
  try {
    ...
    const updated = await updateChangeOrder(actor.companyId, id, co => recordApproval(co, actor, { approved: true }));
    res.status(200).json({ ok: true, changeOrder: updated });
  } catch (error) {
    res.status(422).json({ ok: false, error: error.message || 'Could not approve this change order.' });
  }
}
