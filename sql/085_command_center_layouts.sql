...
--
-- ✅ APPLIED to production (sqhusuuhlmcmkeowdrga) 2026-08-16, on Chris's explicit
-- go-ahead, via Supabase MCP apply_migration as version 085_command_center_layouts.
-- Verified after applying: 7 columns, RLS enabled, 4 policies, 3 indexes, the
-- decisions check constraint, the updated_at trigger, anon has no SELECT, 0 rows,
-- and no new security-advisor findings. The check constraint and the one-active
-- partial unique index were both exercised with rolled-back probe inserts.
--
...
--
-- Before it was applied the Command Center degraded to localStorage-only layout
-- persistence (its behavior before this change) -- the client treats a failing or
-- absent cc_layouts endpoint as "no saved layouts", it does not error. That
-- fallback stays in the client as the offline path.
--
