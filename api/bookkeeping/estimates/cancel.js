// api/bookkeeping/estimates/cancel.js — non-terminal -> cancelled.
// Body: { id, reason }. Always reasoned and kept in history — never a
// silent delete.
...
export default async function handler(req, res) {
  ...
  try {
    const { cancelEstimate } = await _load_estimates();
    ...
    const updated = await updateEstimate(actor.companyId, id, est => cancelEstimate(est, actor, { reason }));
    res.status(200).json({ ok: true, estimate: updated });
  } catch (error) {
    res.status(422).json({ ok: false, error: error.message || 'Could not cancel this estimate.' });
  }
}
