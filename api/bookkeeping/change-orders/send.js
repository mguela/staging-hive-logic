// api/bookkeeping/change-orders/send.js — draft -> sent (estimate) or
// draft -> approved/autoApproved (invoice). Body: { id }.
...
export default async function handler(req, res) {
  ...
  try {
    const { sendChangeOrder } = await _load_change_orders();
    ...
    const updated = await updateChangeOrder(actor.companyId, id, co => sendChangeOrder(co, actor, {}));
    res.status(200).json({ ok: true, changeOrder: updated, qboWritesEnabled: false });
  } catch (error) {
    res.status(422).json({ ok: false, error: error.message || 'Could not send this change order.' });
  }
}
